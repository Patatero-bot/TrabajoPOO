...
class Empleado(Usuario):
    def __init__(self, username, password, nombre_completo, fono, email, direccion,
                 fecha_inicio, salario, cargo, iddepartamento, idusuario=None, id_tipo=2):
    ...
    def guardar(self, conexion: ConexionBD):
        ...
        conexion.ejecutar(
            "INSERT INTO persona (nombre_completo, fono, email, direccion, fecha_inicio, salario, cargo, idusuario, iddepartamento) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (self.nombre_completo, self.fono, self.email, self.direccion,
             self.fecha_inicio, self.salario, self.cargo, self.idusuario, self.iddepartamento)
        )
    ...
    def modificar(conexion: ConexionBD):
        ...
        if not datos:
            ...
            return
        def pmod(campo):
            return input(f"¿Modificar {campo}? [{datos[campo]}] SI/NO: ").lower() == "si"
        ...
        for d in departamentos:
            print(f"{d['iddepartamento']}. {d['nombre']}")
        print("0. Ninguno")
